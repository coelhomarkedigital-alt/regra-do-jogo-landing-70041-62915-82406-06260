...
export const SolutionIntegrated = () => {
  const whyDifficult = ["Ler sozinho exige disciplina que poucos têm naturalmente.", "Sem método estruturado, é fácil perder o fio da meada.", "A falta de acompanhamento faz você desistir no primeiro obstáculo.", "Você não sabe se está extraindo o melhor de cada livro.", "Não há com quem discutir e aprofundar os insights."];
  const differentials = [{
    icon: Target,
...
Porque gera a falsa sensação de que "ler não funciona pra mim".
          ...
          <div className="space-y-4">
            {whyDifficult.map((item, index) => <div key={index} className="flex gap-4 items-start p-5 bg-card rounded-lg border-l-4 border-primary backdrop-blur-sm hover:bg-card/80 transition-colors shadow-card" data-animate="slide-left" data-delay={String(index + 1)}>
            ...
            <Card className="relative bg-gradient-accent border-0 shadow-glow">
              <CardContent className="pt-10 pb-10 px-8">
                <h3 className="text-2xl md:text-3xl font-bold text-accent-foreground text-center mb-8 animate-fade-in">
                  O Clube do livro Regras do Jogo não é só mais um clube de leitura.
                </h3>
                <div className="grid md:grid-cols-3 gap-6">
                  {differentials.map((diff, index) => <div key={index} className="relative bg-background rounded-xl p-5 shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 opacity-0 animate-fade-in overflow-hidden" style={{
                  animationDelay: `${0.6 + index * 0.1}s`
                      ...
                      <div className="relative z-10">
                        ...
                        <p className="text-foreground font-semibold leading-snug">{diff.text}</p>
                      </div>
                    </div>)}
                </div>
              </CardContent>
