      ...
      <div className="container px-4 mx-auto">
        <div className="max-w-5xl mx-auto space-y-12">
          <div className="text-center space-y-4 animate-fade-in">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground" data-animate="fade-up">Tudo foi pensado para caber na sua rotina</h2>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {steps.map((step, index) => <Card key={index} className="bg-card shadow-card hover:shadow-hover transition-all duration-300 border-border/50" data-animate={index % 2 === 0 ? "slide-left" : "slide-right"} data-delay={String(index + 1)}>
            ...
            <Card className="relative bg-gradient-to-r from-[hsl(45,98%,57%)] to-[hsl(38,92%,50%)] border-0 shadow-2xl">
              <CardContent className="pt-8 pb-8 px-8">
                ...
                <div className="grid md:grid-cols-2 gap-4 max-w-3xl mx-auto">
                  {profiles.map((profile, index) => <div key={index} className="bg-white rounded-lg p-4 shadow-md flex items-start gap-3" data-animate="scale" data-delay={String(index + 1)}>
                      <div className="flex-shrink-0 w-6 h-6 rounded-full bg-[hsl(158,64%,52%)] flex items-center justify-center">
                        <Check className="w-4 h-4 text-white" />
                      </div>
                      <p className="text-[#1a1a1a] leading-snug font-medium">
                        {profile}
                      </p>
                    </div>)}
                </div>
